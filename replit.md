...
- **Whop billing webhooks**: Configure a Whop Developer webhook for `membership.activated`, `membership.deactivated`, and `membership.cancel_at_period_end_changed` pointing to `/api/webhooks/whop`, then store its signing secret as the `WHOP_WEBHOOK_SECRET` Replit Secret. The endpoint verifies Standard Webhooks signatures and only invalidates the local billing cache; paid access still re-checks Whop directly.
